...
goalProgress - Word-wrapping goal/task hierarchy view (spec field: rows[] built by goal-tree's buildGoalRows). Self-sizes via the 'contentHeight' aspect like contentBlock; fires 'toggle' with { id } when a goal row's expand gutter is clicked, and 'goalAction' with { id, action } when a row's pause/resume/stop control is clicked (rows carry actions when built with withActions). Used by GoalBrowser and Chat's activity bubble; prefer tree for generic hierarchies.
